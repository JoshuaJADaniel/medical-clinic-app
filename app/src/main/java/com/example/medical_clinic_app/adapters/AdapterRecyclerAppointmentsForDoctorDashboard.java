...
import com.example.medical_clinic_app.AppointmentViewActivityDoctor;
import com.example.medical_clinic_app.DoctorLoginActivity;
...
public class AdapterRecyclerAppointmentsForDoctorDashboard extends RecyclerView.Adapter<AdapterRecyclerAppointmentsForDoctorDashboard.MyViewHolder> {
    private final List<Appointment> appointments;
    private final FormatUsersAppointmentStrategy userFormatStrategy;

    public AdapterRecyclerAppointmentsForDoctorDashboard(List<Appointment> appointments, FormatUsersAppointmentStrategy userFormatStrategy) {
    ...
    public static class MyViewHolder extends RecyclerView.ViewHolder {
        ...
        public MyViewHolder(View view) {
            ...
            view.setOnClickListener(outerView->{

            });
            view.setOnClickListener(innerView -> {

                Intent intent = new Intent(view.getContext(), AppointmentViewActivityDoctor.class);

                intent.putExtra(AppointmentViewActivityDoctor.KEY_PATIENT, patient);
                intent.putExtra(AppointmentViewActivityDoctor.KEY_DOCTOR, doctor);
                intent.putExtra(AppointmentViewActivityDoctor.KEY_TIME, date);
    ...
    @NonNull
    public AdapterRecyclerAppointmentsForDoctorDashboard.MyViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View itemView = LayoutInflater.from(parent.getContext()).inflate(R.layout.list_item_appointment, parent, false);
        return new MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(@NonNull AdapterRecyclerAppointmentsForDoctorDashboard.MyViewHolder holder, int position) {
